export const dietList = [
  {
    id: 1,
    name: "paleo",
    thumb: "./img/diet/paleo.png",
    field: "thay-doi-voc-dang",
    decr: "Paleo là một chế độ ăn giống như những người Thượng Cổ thời xưa. Vốn dĩ đây được xem như là một chế độ ăn kiêng nguyên thuỷ bởi nó chỉ tập trung vào những loại thực phẩm trực tiếp từ thiên nhiên, ăn uống lành mạnh...",
  },
  {
    id: 2,
    name: "keto",
    thumb: "./img/diet/keto.png",
    field: "cai-thien-suc-khoe",
    field: "thay-doi-voc-dang",
    decr: "Chế độ ăn keto là một chế độ ăn ít carb, nhiều chất béo. Nó có thể giúp cơ thể đốt cháy nhiều chất béo hơn, giảm cảm giác đói và giảm lượng đường trong máu. Trong chế độ ăn Keto tiêu chuẩn, tỷ lệ chính xác là 5% carb, 20% protein và 75% chất béo. Ngoài ra còn có một số biến thể khác ví dụ như: - Chế độ Keto theo chu kỳ: bao gồm 5 ngày ăn Ketogenic tiêu chuẩn, sau đó là 2 ngày ăn bù một lượng lớn carbohydrate. - Chế độ Keto mục tiêu: cho phép bạn tăng thêm lượng carbohydrate nếu tham gia luyện tập nặng. - Chế độ ăn Keto giàu protein: với tỷ lệ 60% chất béo, 35% protein và 5% carbohydrate",
  },

  {
    id: 3,
    name: "mediterranean ",
    thumb: "./img/diet/low-carb.png",
    field: "dinh-duong-benh-ly",
    field: "cai-thien-suc-khoe",
    decr: "Chế độ ăn kiêng này yêu cầu giảm tiêu thụ thịt đỏ, đường và chất béo bão hòa. Thay vào đó, chế độ ăn kiêng này khuyên bạn nên ưu tiên tiêu thụ trái cây, rau, các loại hạt và protein nạc, chẳng hạn như cá.",
  },

  {
    id: 4,
    name: "dash",
    thumb: "./img/diet/dash.png",
    field: "dinh-duong-benh-ly",
    field: "cai-thien-suc-khoe",
    decr: "DASH là từ viết tắt của 'chế độ ăn kiêng tiếp cận để ngăn chặn tăng huyết áp' và được thúc đẩy bởi Viện Tim, Phổi và Máu Quốc gia. Chế độ ăn kiêng này đã xuất hiện từ những năm 1990 và được tạo ra, như tên gọi của nó, để giúp mọi người kiểm soát huyết áp cao. DASH kêu gọi tăng cường tiêu thụ thực phẩm ít natri, nhiều chất xơ và không khuyến khích thực phẩm có nhiều chất béo bão hòa, natri và đường bổ sung.",
  },
  {
    id: 5,
    name: "flexitarian",
    thumb: "./img/diet/flexitarian.jpg",
    field: "dinh-duong-benh-ly",
    field: "cai-thien-suc-khoe",
    decr: "Chế độ ăn kiêng linh hoạt đã trở nên vô cùng phổ biến vào năm 2020. Như tên gọi, đó là một cách tiếp cận linh hoạt đối với chế độ ăn chay. Nói cách khác, chế độ ăn kiêng này cho phép bạn vẫn ăn thịt nhưng chỉ thỉnh thoảng ăn. Không có bất kỳ quy tắc nghiêm ngặt nào cho chế độ ăn kiêng này, điều này giúp bạn dễ dàng tuân theo hơn.",
  },
  {
    id: 6,
    name: "vegetarian",
    thumb: "./img/diet/vegan.jpg",
    field: "dinh-duong-benh-ly",
    field: "cai-thien-suc-khoe",
    decr: "chế độ ăn chay, như bạn có thể đã biết, có nhiều kiểu ăn chay. Kiểu phổ biến nhất là lacto-ovo, phần lớn là ăn thực phẩm có nguồn gốc thực vật, tuy nhiên cho phép tiêu thụ các sản phẩm từ sữa và trứng; không ăn thịt hoặc thủy - hải sản.",
  },
];

export const recipesList = [
  {
    id: 1,
    name: "CÁCH LÀM GÀ ĂN MÀY CAO SANG QUYỀN QUÝ",
    thumb: "./img/recipes/ga-an-may.png",
    auth: "Hà Vy",
    day: "13",
    month: "March",
    date: "March 13, 2021",
    duration: 45,
    season: "spring",
    region: "north",
    category: "main",
    method: "Nướng",
  },

  {
    id: 2,
    name: "HÀU NƯỚNG PHOMAI NGON NHẤT THẾ GIỚI",
    thumb: "./img/recipes/hau-nuong.png",
    auth: "Hà Vy",
    day: "13",
    month: "March",
    date: "March 13, 2021",
    duration: 45,
    season: "summer",
    region: "north",
    category: "sub",
    method: "Nướng",
  },
  {
    id: 3,
    name: "XÔI GÀ",
    thumb: "./img/recipes/xoi-ga.png",
    auth: "Hà Vy",
    day: "13",
    month: "March",
    date: "March 13, 2021",
    duration: 45,
    season: "fall",
    region: "north",
    category: "sub",
    method: "Hấp",
  },

  {
    id: 4,
    name: "CÁCH LỌC XƯƠNG GÀ LUỘC CỰC DỄ",
    thumb: "./img/recipes/ga-luoc.png",
    auth: "Hà Vy",
    day: "13",
    month: "March",
    date: "March 13, 2021",
    duration: 45,
    season: "winter",
    region: "north",
    category: "main",
    method: "boil",
  },

  {
    id: 5,
    name: "CÁCH MUỐI CÁ HỒI RUBY SIÊU TO KHỔNG LỒ",
    thumb: "./img/recipes/ca-hoi.png",
    auth: "Hà Vy",
    day: "13",
    month: "March",
    date: "March 13, 2021",
    duration: 45,
    season: "spring",
    region: "north",
    category: "sub",
    method: "luoc",
  },

  {
    id: 6,
    name: "CÁ TRẮM KHO MĂNG CAY BẰNG NỒI ÁP SUẤT",
    thumb: "./img/recipes/ca-tram.png",
    auth: "Hà Vy",
    day: "13",
    month: "March",
    date: "March 13, 2021",
    duration: 45,
    season: "summer",
    region: "north",
    category: "main",
    method: "luoc",
  },

  {
    id: 7,
    name: "CÁ VƯỢC OM XỐT YOSENABE KIỂU NHẬT",
    thumb: "./img/recipes/ca-vuoc.png",
    auth: "Hà Vy",
    day: "13",
    month: "March",
    date: "March 13, 2021",
    duration: 45,
    season: "fall",
    region: "north",
    category: "main",
    method: "luoc",
  },

  {
    id: 8,
    name: "THỊT KHÂU NHỤC KIỂU QUẢNG ĐÔNG",
    thumb: "./img/recipes/khau-nhuc.png",
    auth: "Hà Vy",
    day: "13",
    month: "March",
    date: "March 13, 2021",
    duration: 45,
    season: "winter",
    region: "north",
    category: "main",
    method: "luoc",
  },
  {
    id: 9,
    name: "CÁCH LÀM GÀ NƯỚNG CAY XÈ PIRI PIRI",
    thumb: "./img/recipes/ga-nuong.png",
    auth: "Hà Vy",
    day: "13",
    month: "March",
    date: "March 13, 2021",
    duration: 45,
    season: "spring",
    region: "north",
    category: "main",
    method: "luoc",
  },
  {
    id: 10,
    name: "MẸO CHIÊN CÁ GIÒN TAN Bằng NCKD",
    thumb: "./img/recipes/ca-chien.png",
    auth: "Hà Vy",
    day: "13",
    month: "March",
    date: "March 13, 2021",
    duration: 45,
    season: "summer",
    region: "north",
    category: "main",
    method: "luoc",
  },
  {
    id: 11,
    name: "TỰ LÀM THỊT QUAY GIÒN BÌ",
    thumb: "./img/recipes/thit-quay.png",
    auth: "Hà Vy",
    day: "13",
    month: "March",
    date: "March 13, 2021",
    duration: 45,
    season: "fall",
    region: "north",
    category: "main",
    method: "luoc",
  },
  {
    id: 12,
    name: "BÁNH MỲ TRỨNG ỐP LA",
    thumb: "./img/recipes/thit-quay.png",
    auth: "Hà Vy",
    day: "13",
    month: "March",
    date: "March 13, 2021",
    duration: 30,
    season: "fall",
    region: "north",
    category: "sub",
    method: "luoc",
  },
  {
    id: 13,
    name: "THỊT BÒ SỐT TIÊU ĐEN",
    thumb: "./img/recipes/bo-sot-tieu.png",
    auth: "Vinh Trần",
    day: "15",
    month: "August",
    date: "August 15, 2022",
    duration: 20,
    season: "winter",
    region: "north",
    category: "main",
    method: "fry",
  },
  {
    id: 14,
    name: "BÁNH MỲ NƯỚNG BƠ TỎI",
    thumb: "./img/recipes/banh-my-nuong-bo-toi.png",
    auth: "Hà Đặng",
    day: "18",
    month: "Jul",
    date: "Jul 18, 2021",
    duration: 15,
    season: "winter",
    region: "north",
    category: "cake",
    method: "grill",
  },
  {
    id: 15,
    name: "MỲ XÀO RAU CỦ",
    thumb: "./img/recipes/my-xao-rau-cu.png",
    auth: "Dương Thái",
    day: "10",
    month: "July",
    date: "July 10 2021",
    duration: 25,
    season: "fall",
    region: "south-side",
    category: "sub",
    method: "fried",
  },
  {
    id: 16,
    name: "BÁNH MỲ SANWICH",
    thumb: "./img/recipes/sanwich.png",
    auth: "Khánh Trần",
    day: "18",
    month: "September",
    date: "September 18, 2020",
    duration: 15,
    season: "summer",
    region: "north-side",
    category: "cake",
    method: "grill",
  },
  {
    id: 17,
    name: "CƠM CHIÊN THẬP CẨM",
    thumb: "./img/recipes/com-chien.png",
    auth: "Tuấn Đặng",
    day: "16",
    month: "December",
    date: "December 16, 2021",
    duration: 18,
    season: "winter",
    region: "south-side",
    category: "main",
    method: "fried",
  },
  {
    id: 18,
    name: "BÁNH MỲ TRỨNG ỐP LA",
    thumb: "./img/recipes/banh-my-trung.png",
    auth: "Hà Trần",
    day: "19",
    month: "Febuary",
    date: "Febuary 19, 2020",
    duration: 15,
    season: "summer",
    region: "south-side",
    category: "summer",
    method: "grill",
  },
  {
    id: 19,
    name: "BÁNH MỲ KAYA TOAST",
    thumb: "./img/recipes/banh-brazil.png",
    auth: "Ngọc Hân",
    day: "22",
    month: "March",
    date: "March 22, 2022",
    duration: 30,
    season: "winter",
    region: "south-side",
    category: "cake",
    method: "grill",
  },
  {
    id: 20,
    name: "NGŨ CỐC CÁC LOẠI",
    thumb: "./img/recipes/ngu-coc.png",
    auth: "Tú Trần",
    day: "01",
    month: "March",
    date: "March 01, 2019",
    duration: 10,
    season: "fall",
    region: "south-side",
    category: "sub",
    method: "salad",
  },
  {
    id: 21,
    name: "GỎI CỦ HŨ DỪA",
    thumb: "./img/recipes/cu-hu-dua.png",
    auth: "Khánh Đặng",
    day: "21",
    month: "July",
    date: "July 21, 2020",
    duration: 45,
    season: "summer",
    region: "south-side",
    category: "appetizer",
    method: "salad",
  },
  {
    id: 22,
    name: "GỎI NGÓ SEN TÔM THỊT",
    thumb: "./img/recipes/goi-ngo-sen-tom-thit.png",
    auth: "Huệ Đặng",
    day: "25",
    month: "July",
    date: "July 25, 2020",
    duration: 45,
    season: "summer",
    region: "south-side",
    category: "appetizer",
    method: "salad",
  },
  {
    id: 23,
    name: "GỎI HOA CHUỐI",
    thumb: "./img/recipes/goi-hoa-chuoi.png",
    auth: "Huệ Hồng",
    day: "25",
    month: "August",
    date: "August 25, 2020",
    duration: 50,
    season: "summer",
    region: "north-side",
    category: "appetizer",
    method: "salad",
  },
  {
    id: 24,
    name: "GỎI BÒ MÈ BÓP THẤU",
    thumb: "./img/recipes/bo-me-bop-thau.png",
    auth: "Diệp Anh",
    day: "18",
    month: "August",
    date: "August 18, 2022",
    duration: 50,
    season: "summer",
    region: "south-side",
    category: "appetizer",
    method: "salad",
  },
  {
    id: 25,
    name: "GỎI BÒ MÈ BÓP THẤU",
    thumb: "./img/recipes/thit-nguoi.png",
    auth: "Lan Anh",
    day: "05",
    month: "Jul",
    date: "Jul 05, 2022",
    duration: 50,
    season: "summer",
    region: "central",
    category: "appetizer",
    method: "salad",
  },
  {
    id: 26,
    name: "CÀNG CUA BÁCH HOA",
    thumb: "./img/recipes/cang-cua-bach-hoa.png",
    auth: "Giang Trần",
    day: "17",
    month: "Jul",
    date: "Jul 17, 2022",
    duration: 50,
    season: "summer",
    region: "central",
    category: "appetizer",
    method: "salad",
  },
  {
    id: 27,
    name: "TRÀ SỮA BÍ ĐAO",
    thumb: "./img/recipes/tra-bi-dao.png",
    auth: "Huệ Anh",
    day: "17",
    month: "October",
    date: "October 17, 2022",
    duration: 60,
    season: "summer",
    region: "south-side",
    category: "beverage",
    method: "boil",
  },
  {
    id: 28,
    name: "HẠT ĐÁC RIM DỨA & CHANH LEO",
    thumb: "./img/recipes/hat-dac.png",
    auth: "Loan Phạm",
    day: "21",
    month: "October",
    date: "October 21, 2022",
    duration: 60,
    season: "summer",
    region: "north-side",
    category: "beverage",
    method: "boil",
  },
  {
    id: 29,
    name: "CÁCH LÀM KOMBUCHA ĐÀO “LƯỜI”",
    thumb: "./img/recipes/dao-luoi.png",
    auth: "hà Phạm",
    day: "21",
    month: "November",
    date: "November 21, 2022",
    duration: 60,
    season: "summer",
    region: "north-side",
    category: "beverage",
    method: "boil",
  },
  {
    id: 30,
    name: "CÁCH LÀM LASSI “SỮA CHUA LẮC KIỂU ẤN”",
    thumb: "./img/recipes/lac-an.png",
    auth: "Hoa Lan",
    day: "21",
    month: "Febuary",
    date: "Febuary 17, 2021",
    duration: 60,
    season: "summer",
    region: "north-side",
    category: "beverage",
    method: "boil",
  },

  {
    id: 31,
    name: "CÀ PHÊ ĐÁ XAY CỐT DỪA THẠCH CARAMEL ",
    thumb: "./img/recipes/ca-phe.png",
    auth: "Tuấn Khanh",
    day: "19",
    month: "Febuary",
    date: "Febuary 19, 2022",
    duration: 15,
    season: "summer",
    region: "south-side",
    category: "beverage",
    method: "boil",
  },

  {
    id: 32,
    name: "CHÈ ĐẬU ",
    thumb: "./img/recipes/che-dau.png",
    auth: "Trang Thu",
    day: "16",
    month: "October",
    date: "October 16, 2021",
    duration: 50,
    season: "summer",
    region: "central",
    category: "dessert",
    method: "tea",
  },
  {
    id: 33,
    name: "CHÈ VẢI HẠT SEN ",
    thumb: "./img/recipes/che-vai-hat-sen.png",
    auth: "Nghĩa Lưu",
    day: "20",
    month: "Jul",
    date: "Jul 20, 2021",
    duration: 50,
    season: "summer",
    region: "south-side",
    category: "dessert",
    method: "tea",
  },
  {
    id: 33,
    name: " BÁNH CHUỐI NƯỚNG",
    thumb: "./img/recipes/banh-chuoi-nuong.png",
    auth: "Ninh Lan",
    day: "20",
    month: "Jul",
    date: "Jul 20, 2021",
    duration: 40,
    season: "winter",
    region: "central",
    category: "dessert",
    method: "grill",
  },
  {
    id: 34,
    name: "BÁNH DA LỢN",
    thumb: "./img/recipes/banh-da-lon.png",
    auth: "Ninh Lan",
    day: "19",
    month: "May",
    date: "May 19, 2022",
    duration: 60,
    season: "summer",
    region: "central",
    category: "dessert",
    method: "cake",
  },
  {
    id: 35,
    name: "SƯƠNG SÁO NƯỚC CỐT DỪA",
    thumb: "./img/recipes/suong-sao-cot-dua.png",
    auth: "Phan Anh",
    day: "30",
    month: "August",
    date: "August 30, 2020",
    duration: 120,
    season: "summer",
    region: "central",
    category: "dessert",
    method: "beverage",
  },
  {
    id: 36,
    name: "BÁNH CHUỐI HẤP NƯỚC CỐT DỪA",
    thumb: "./img/recipes/banh-chuoi-hap.png",
    auth: "Phan Anh",
    day: "27",
    month: "January",
    date: "January 27, 2020",
    duration: 45,
    season: "summer",
    region: "central",
    category: "dessert",
    method: "boil",
  },
];

export const suggestList = [
  {
    id: 1,
    category: "cake",
    name: "Bánh Ngọt",
    thumb: "./img/home/suggest/banh-ngot.png",
  },

  {
    id: 2,
    category: "beverage",
    name: "Đồ uống",
    thumb: "./img/home/suggest/drink.png",
  },
  {
    id: 3,
    category: "appetizer",
    name: "Khai vị",
    thumb: "./img/home/suggest/khai-vi.png",
  },
  {
    id: 4,
    category: "main",
    name: "Món chính",
    thumb: "./img/home/suggest/mon-chinh.png",
  },
  {
    id: 5,
    category: "sub",
    name: "Món phụ",
    thumb: "./img/home/suggest/mon-phu.png",
  },
  {
    id: 6,
    category: "dessert",
    name: "Tráng miệng",
    thumb: "./img/home/suggest/trang-mieng.png",
  },
];

export const courseList = [
  {
    id: 1,
    name: "BỮA SÁNG THÔNG MINH",
    thumb: "/img/course/bua-sang-thong-minh.png",
    price: 1700000,
    decr: " Khóa học cung cấp công thức chế biến món ăn sáng ngon,phương pháp nấu tiết kiệm thời gian nhưng đảm bảo dinh dưỡng.",
  },
  {
    id: 2,
    name: "MÓN NGON BA MIỀN",
    thumb: "/img/course/mon-ngon-ba-mien.png",
    price: 1800000,
    decr: " Tham gia lớp học, bạn sẽ tích lũy được kiến thức, kỹ năng nấu nhiều món ăn hấp dẫn từ Chuyên gia",
  },
  {
    id: 3,
    name: "HÔM NAY ĂN GÌ",
    thumb: "/img/course/hom-nay-an-gi.png",
    price: 1600000,
    decr: " Khóa học giúp bạn có thể biến nguyên liệu quen thuộc thành món ăn thơm ngon, lạ miệng, làm phong phú thực đơn.",
  },

  {
    id: 4,
    name: "ĐIỂM TÂM HONGKONG",
    thumb: "/img/course/hong-kong.png",
    price: 1600000,
    decr: "Các món ăn đặc trưng của xứ Cảng Thơm để làm phong phú hơn bữa sáng cho cả gia đình của bạn.",
  },
  {
    id: 5,
    name: "MÓN ĂN ĐƯỜNG PHỐ",
    thumb: "/img/course/mon-an-duong-pho.png",
    price: 1600000,
    decr: "Giúp bạn thấu hiểu cặn kẽ cách chế biến thành công các món ăn vặt đường phố đặc trưng của Việt Nam.",
  },

  {
    id: 6,
    name: "MÓN ĂN ĐƯỜNG PHỐ",
    thumb: "/img/course/mon-an-duong-pho.png",
    price: 1600000,
    decr: "Giúp bạn thấu hiểu cặn kẽ cách chế biến thành công các món ăn vặt đường phố đặc trưng của Việt Nam.     ",
  },
  {
    id: 7,
    name: "MÓN ĂN BỔ DƯỠNG",
    thumb: "/img/course/mon-an-bo-duong.png",
    price: 1600000,
    decr: "Giúp bạn nắm vững kiến thứ chế biến nhiều món ngon bổ sung dưỡng chất, nâng cao sức khỏe cho người thân.",
  },
  {
    id: 8,
    name: "BÁNH PAPPAROTI",
    thumb: "/img/course/paroti.png",
    price: 500000,
    decr: "Nhiều người nghĩ rằng làm bánh papparoti thật khó, nhưng bất ngờ là món bánh này lại có công thức khá đơn giản. Bạn chỉ cần một chút bí quyết, một chút mẹo nhỏ là có thể tạo ra những chiếc bánh thơm lừng.",
  },
];

export const blogList = [
  {
    id: 1,
    name: "Vét tủ lạnh HÔ BIẾN BÁNH CHƯNG Ế THÀNH KIMBAP CHIÊN GIÒN NGON TUYỆT",
    thumb: "./img/blog/banh_chung.png",
    decr: "[Tạm biệt Tết] x Vét tủ lạnh HÔ BIẾN BÁNH CHƯNG Ế THÀNH KIMBAP CHIÊN GIÒN NGON TUYỆT Valentine xong rồi, gấp poster lại chưa các bạn ơi? Người ế thì mình chịu. Còn bánh chưng ế thì ship ngay…",
    auth: "February 26, 2019",
    date: "Tâm Phạm",
  },
  {
    id: 2,
    name: "CƠN BÃO “HÀN QUỐC – HÀNH TRÌNH MỸ VỊ” CÀN QUÉT CÁC BỮA TRƯA TẠI MỘT CÔNG SỞ NỌ TẠI HÀ NỘI",
    thumb: "./img/blog/han_quoc.png",
    decr: "CƠN BÃO “HÀN QUỐC – HÀNH TRÌNH MỸ VỊ” CÀN QUÉT CÁC BỮA TRƯA TẠI MỘT CÔNG SỞ NỌ TẠI HÀ NỘI Vâng thưa các bạn, kể từ ngày 10/10/2021 tới nay, một cơn bão lạ mang tên “Hàn Quốc…",
    auth: "March 15, 2022",
    date: "Hương Lan",
  },
  {
    id: 3,
    name: "CÁCH LÀM MIẾN CUỘN GIMMARI – MÓN ĂN VẶT KHÔNG DỪNG ĐƯỢC MIỆNG KIỂU HÀN QUỐC",
    thumb: "./img/blog/mien-cuon.png",
    decr: "CÁCH LÀM MIẾN CUỘN GIMMARI – MÓN ĂN VẶT KHÔNG DỪNG ĐƯỢC MIỆNG KIỂU HÀN QUỐC Có một bí ẩn cần YBer giải đáp giúp mình… Rằng Thử thách “Hàn Quốc: Hành Trình Mỹ Vị” đã đóng cổng nhận bài…",
    auth: "March 6, 2022",
    date: "Hoa Nguyễn",
  },
  {
    id: 4,
    name: "CÁCH LÀM JAPCHAE – KHÁM PHÁ MÓN CUNG ĐÌNH HÀN QUỐC",
    thumb: "./img/blog/japchae.png",
    decr: "CÁCH LÀM JAPCHAE – KHÁM PHÁ MÓN CUNG ĐÌNH HÀN QUỐC Japchae, được biết đến ở Việt Nam dưới cái tên miến xào rau củ, là một trong những món ăn truyền thống của Hàn Quốc nổi danh trên bản…",
    auth: "February 7, 2022",
    date: "Diệp Anh",
  },
  {
    id: 5,
    name: "GIẢI MÃ DALGONA HOT TREND YÊU BẾP “KHUẤY ĐẢO” MẠNG XÃ HỘI",
    thumb: "./img/blog/dalgona.png",
    decr: "GIẢI MÃ DALGONA HOT TREND YÊU BẾP “KHUẤY ĐẢO” MẠNG XÃ HỘI Xin chào các bạn, lần tổng kết hot trend này, spotlight của chúng ta sẽ dành trọn cho gương mặt vàng trong làng “khuấy đảo” mang tên Cà…",
    auth: "November 3, 2022",
    date: "Ngô Huệ",
  },
  {
    id: 6,
    name: "CÁCH LÀM NOKDU BINDAETTEOK – BÁNH KẾP ĐẬU XANH KIỂU HÀN",
    thumb: "./img/blog/dau.png",
    decr: "Nhắc đến Hàn Quốc, người ta không khỏi nghĩ ngay đến kimchi – món ăn có thể coi là một đại diện tiêu biểu cho quốc gia này trên bản đồ ẩm thực thế giới. Theo ghi nhận của Bảo Tàng Kim Chi (Museum Kimchikan) tại Seoul, Hàn Quốc thì có tới 187 loại kimchi từ xưa đến nay. Bên cạnh kimchi cải thảo đã nổi tiếng khắp thế giới, Hàn Quốc còn có một số loại kimchi phổ biến làm từ củ cải, lá hẹ, bắp cải, dưa chuột v.v. Trong series “Muối cả thế giới” của Esheep Kitchen, chúng mình cũng đã ghé thăm Hàn Quốc và Nhật Bản để học lỏm cách làm dưa chuột muối đấy! Thử trải nghiệm làm 1/187 loại kimchi với công thức này nhé",
    auth: "September 7, 2021",
    date: "Lan Anh",
  },
  {
    id: 7,
    name: "My little garden – Where is your soul’s shelter?",
    thumb: "./img/blog/little-garden.png",
    decr: "My little garden 🌿 Where is your soul’s shelter?Đôi khi ai cũng cần những góc nhỏ tĩnh lặng cho riêng mình.Góc nhỏ của bạn là nơi nào?",
    auth: "May 25, 2022",
    date: "Quốc Anh",
  },
  {
    id: 8,
    name: "Vẻ đẹp của rau củ",
    thumb: "./img/blog/rau-cu.png",
    decr: "Vẻ đẹp của rau củ Bất cứ ở đâu, bất cứ công việc nào, nếu bạn toàn tâm toàn ý và dành niềm hứng thú say mê với nó, bạn cũng có thể tìm được vẻ đẹp của nó đấy!…",
    auth: "May 14, 2021",
    date: "Quốc Anh",
  },
  {
    id: 9,
    name: "[Cuối tuần vào bếp cùng con] BÁNH NGÔ MÙA HÈ – SIÊU NGON SIÊU DỄ",
    thumb: "./img/blog/banh-ngo.png",
    decr: "Mình học món này của bạn Hoàng Minh đã chia sẻ với Yêu Bếp.Bạn ấy làm xong mang tặng studio bọn mình. Nó siêu ngon đến mức mình ngay lập tức ghim lại cách làm để cuối tuần cùng bọn Mĩm Cừu làm.Và đây – Một mẻ bánh ngô tuyệt ngon cho bữa xế nhiều nay của nhà mình – kèm trà sữa Thái đỏ trân châu đường đen bạn Mĩm tự làm đãi cả nhà.",
    auth: "May 14, 2021",
    date: "Quốc Anh",
  },
  {
    id: 10,
    name: "Màu của khu vườn, màu của những ngày đầu hạ…",
    thumb: "./img/blog/vuon.png",
    decr: "Màu của khu vườn, màu của những ngày đầu hạ. XANH MƯỚT Điều mà bọn mình yêu thích nhất ở Studio là gì? Là có một khu vườn trên mây. Một khoảnh vườn nhỏ xinh xắn và một ban công…",
    auth: "November 3, 2022",
    date: "Ngô Huệ",
  },
  {
    id: 11,
    name: "[Khu vườn mùa hạ]",
    thumb: "./img/blog/vuon-ha.png",
    decr: "[Khu vườn mùa hạ] Hôm nay, khu vườn trên mây của chúng mình có “khách quý” đến thăm. Một người bạn thân quen từ Yêu Bếp. Theo mình nhớ thì ấy đã follow mình tới 7 năm nay, vậy mà…",
    auth: "March 6, 2022",
    date: "Hoa Nguyễn",
  },
  {
    id: 12,
    name: "VU VƠ",
    thumb: "./img/blog/vu-vo.png",
    decr: "Chẳng biết giờ ương dở này up một chiếc hình vu vơ lên thì có ai xem không!!! Vu vơ mà ngon không tưởng!!!! Huhu!!! Lọ mọ thử làm bò hầm vang đỏ kiểu Pháp (Boeuf Bourguignon) kiểu của cô…",
    auth: "February 7, 2022",
    date: "Diệp Anh",
  },
  {
    id: 13,
    name: "TẾT ĐOAN NGỌ THÁNG NĂM",
    thumb: "./img/blog/doan-ngo.png",
    decr: "Những sớm tháng 5 mát mẻ, bọn trẻ con sẽ thật háo hức chờ đến ngày “giết sâu bọ” để thi nhau mở cái liếp giang trên liễn gốm ủ rượu nếp, để ngửi mùi hơi rượu sộc thẳng lên…",
    auth: "March 15, 2022",
    date: "Hương Lan",
  },
  {
    id: 14,
    name: "MÙA VỤ CÀ CHUA – CÁCH LÀM SUN DRIED TOMATO",
    thumb: "./img/blog/ca-chua.png",
    decr: "Cuối tuần vừa rồi, nhân ngày nắng ráo, mình đã dỡ giàn cà chua và thu hoạch đợt cà chua cuối. Nhìn những trái cà chua chín mọn và thơm mát này, không thể buồn được. Không thể buồn được. Cảm giác niềm vui cứ len lỏi mãi trên tay, khi chạm vào sự mọng nước, sự nhẵn mịn bóng bẩy thơm tho mùi đất này. Niềm vui sáng rỡ lên cả khi nín thở xếp từng trái, từng trái vào chiếc giỏ con. Xếp nhẹ nhàng, vui nhẹ nhàng.",
    auth: "February 26, 2019",
    date: "Tâm Phạm",
  },
  {
    id: 15,
    name: "NHỮNG NIỀM VUI NHÈ NHẸ",
    thumb: "./img/blog/happy.png",
    decr: "Những chiếc bánh nướng kiểu Anh và buổi chiều quá đỗi bình yên. Buổi học làm bánh chiều t7 16.3.2022. Một buổi chiều khiến tôi sẽ nhớ mãi. Gió mát lịm ngoài cửa sổ Studio và tất thảy chúng tôi…",
    auth: "February 7, 2022",
    date: "Diệp Anh",
  },
  {
    id: 16,
    name: "MÙA THU ĐẦU TIÊN Ở STUDIO",
    thumb: "./img/blog/studio.png",
    decr: "Mùa thu đầu tiên ở Studio🍁 Là mùa thu có nắng vàng, có góc ban công thật rộng để treo những quả hồng vàng ruộm, có những mẹt lá vối hong nắng để mời khách ghé thăm và có một…",
    auth: "March 6, 2022",
    date: "Lu Nguyễn",
  },
  {
    id: 17,
    name: "KHU VƯỜN RỰC RỠ ",
    thumb: "./img/blog/ruc-ro.png",
    decr: "Trong lúc chị đại Sheep đang vi vu trời Tây Ban Nha, thưởng thức toàn món ngon của lạ 😡😡😡 thì mấy đứa chúng mình lại tranh thủ một ngày Hà Nội nắng đẹp để “quy hoạch” lại khu vườn yêu…",
    auth: "September 6, 2022",
    date: "Phan Anh",
  },
  {
    id: 18,
    name: "",
    thumb: "./img/blog/mien-cuon.png",
    decr: "",
    auth: "December 6, 2022",
    date: "Đặng Tuấn",
  },
  {
    id: 19,
    name: "(GÓC REVIEW) PIZZA CHẢ CÁ",
    thumb: "./img/blog/pizza.png",
    decr: "Ok, hôm qua mọi người đã được một phen náo loạn vì PIZZA MẮM TÔM mình up lên group Hội Ăn Gì rồi.Nên hôm nay, mình quyết định đi qua tiệm Pizza 4P’s ở Phan Kế Bính, HN – nơi được cho là có món PIZZA MẮM TÔM ấy ở HN ăn thử xem thế nào.",
    auth: "March 6, 2022",
    date: "Thu Trang",
  },
  {
    id: 20,
    name: "(GÓC REVIEW) 🌿BÚN ỐC NGON KHÔNG MÌ CHÍNH🌿",
    thumb: "./img/blog/bun-oc.png",
    decr: "Suýt nữa thì tuyệt vọng. Chẳng còn tìm được hàng bún ốc nào vừa miệng mình mà lại không/hoặc ít mì chính.Tự dưng hôm nay thằng em lại vui chân rủ đi ăn bún ốc.Một hàng nghe danh nổi đã lâu nhưng thế nào lại không đến tai mình. =))",
    auth: "March 6, 2022",
    date: "Oanh Han",
  },
  {
    id: 21,
    name: "TỪ A-Z CÁCH LÀM KIM CHI TRUYỀN THỐNG HÀN QUỐC",
    thumb: "./img/blog/kim-chi.png",
    decr: "Không chỉ đơn thuần là một món ăn truyền thống, kimchi còn là gắn liền với bề dày lịch sử, văn hóa và con người của xứ sở này. Và nếu bạn chưa biết thì kimchi và kimjang – văn hóa muối kimchi của Hàn Quốc đã được UNESCO chính thức công nhận vào danh mục di sản văn hóa phi vật thể đại diện của nhân loại đó!",
    auth: "March 21, 2022",
    date: "Diep Phạm",
  },
];
